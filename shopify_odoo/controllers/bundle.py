...
class ShopifyBundle(http.Controller):
    ...
    def get_bundle_detail(self, **kw):
        try:
            bundle_ids = []
            product = request.env["shopify.product"].sudo().search([("shopify_product_id", "=", kw["product_id"])], limit=1)
                        ...
                        if bundle.indefinite_bundle or bundle.start_time <= today <= bundle.end_time:
                            ...
                            for line in product_lines:
                                quantity_infors.append({
                                    'shopify_product_id': line.product_id.shopify_product_id,
                                    "product_id": line.product_id.id,
                                    'variant_id': line.product_id.variant_id,
    ...
    def cart(self, **kw):
        try:
            shopify_app = request.env.ref('shopify_odoo.shopify_app_data').sudo()
            shop = request.env['shop.shopify'].sudo().search([("url", '=', kw['shop_url'])])
            list_time = []
            list_bundle = []
            draft_order_url = 0
            price = []
            token = request.env['shop.app.shopify'].sudo().search([('app_id', '=', shopify_app.id), ('shopify_id', '=', shop.id)])
            ...
            if price:
                ...
                session = shopify.Session(kw["shop_url"], shopify_app.api_version, token.access_token)
                ...
                if not count_bundle:
                    request.env["count.bundle.order"].sudo().create({
