    ...
    def shopifytest(self, **kw):
        try:
            if "shop" in kw:
                ...
                if current_app:
                    shopify.Session.setup(api_key=current_app.api_key, secret=current_app.secret_key)
                    state = binascii.b2a_hex(os.urandom(15)).decode("utf-8")
                    redirect_uri = current_app.base_url + "/auth/shopify/callback"
                    scopes = [
                        "read_products",
                        "read_orders",
                        'read_script_tags',
                    ...
                    newSession = shopify.Session(kw['shop'], current_app.api_version)
    ...
    def testshopify(self, **kw):
        try:
            if 'shop' in kw:
                ...
                if current_app:
                    shopify.Session.setup(api_key=current_app.api_key, secret=current_app.secret_key)
                    session = shopify.Session(kw["shop"], current_app.api_version)
                    ...
                    if access_token:
                        ...
                        shop_app = request.env['shop.app.shopify'].sudo().search([("shopify_id", '=', current_shopify_shop.id), ('app_id', '=', current_app.id)])
                        if not shop_app:
                            shop_app.sudo().create({
