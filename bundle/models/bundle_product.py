...
class BundleProduct(models.Model):
    ...
    def get_list_bundle_total(self):
        self.env.cr.execute(
            "SELECT product_bundle_id FROM bundle_total_product_rel WHERE product_template_id = %s" % self.id)
        bundle_ids = (x[0] for x in self.env.cr.fetchall())
        return bundle_ids

    def get_list_bundle_product(self):
        self.env.cr.execute(
            "SELECT product_bundle_id FROM bundle_each_product_rel WHERE product_template_id = %s" % self.id)
        bundle_ids = (x[0] for x in self.env.cr.fetchall())
        return bundle_ids

    def get_list_bundle_tier(self):
        self.env.cr.execute(
            "SELECT product_bundle_id FROM bundle_tier_product_rel WHERE product_template_id = %s" % self.id)
        bundle_ids = (x[0] for x in self.env.cr.fetchall())
        return bundle_ids
